package properties;

interface DefaultValues {

	static final String DEFAULT_PROPERTIES = "default.properties";

	static final String DEFAULT_LANG = "default_lang";

	static final String DEFAULT_TIMEOUT_JOINING_PHASE = "default_timeout_joining_phase";
	static final String DEFAULT_TIMEOUT_MUSH_ATTACK_PHASE = "default_timeout_mush_attack_phase";

	static final String DEFAULT_GAME_MUSH_PROPORTION = "default_mush_proportion";
	static final String DEFAULT_GAME_MIN_MUSH_AMOUNT = "default_min_mush_amount";
	static final String DEFAULT_GAME_REVOTING_ALLOWED = "default_revoting_allowed";
}
